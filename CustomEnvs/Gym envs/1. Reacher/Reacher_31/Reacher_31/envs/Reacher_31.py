...
class model_data:
    ...
    def __init__(self):
        self.num_links = 3
        self.link_lengths = [0.08,0.11,0.12]
...
class Reacher_v31(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, add_dummy=False, max_links=5):
        ...
        mujoco_env.MujocoEnv.__init__(self, 'Reacher_31.xml', 2)

    def step(self, a):
        vec = self.get_body_com("fingertip")-self.get_body_com("target")
        reward_dist = - np.linalg.norm(vec)
        reward_ctrl = - np.square(a).sum()/3
