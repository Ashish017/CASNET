...
class Reacher_v20(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, add_dummy=False, max_links=5):
        ...
        mujoco_env.MujocoEnv.__init__(self, 'Reacher_20.xml', 2)

    def step(self, a):
        vec = self.get_body_com("fingertip")-self.get_body_com("target")
        reward_dist = - np.linalg.norm(vec)
        reward_ctrl = - np.square(a).sum()/2
    ...
    def _set_range(self, qpos):
    	if qpos >= 0.0:
    		return qpos % (2*math.pi)
    	else:
    		return qpos % -(2*math.pi)
    ...
    def _get_obs(self):
        ...
        for link_number in range(modelData.num_links):
            ...
            joint_pos.append(self._set_range(float(self.data.qpos[joint_qpos_addr])))
