...
class model_data:
	...
	def __init__(self):
		self.num_legs = 6
		self.leg_lengths = [[0.2828, 0.2828, 0.5656],[0.2828, 0.2828, 0.5656],[0.2828,0.2828, 0.5656],[0.2828, 0.2828, 0.5656],[0.2828, 0.2828, 0.5656],[0.2828, 0.2828, 0.5656]]
		self.leg_starts = [[0.1414, 0.2449],[0.2828, 0.0],[0.1414, -0.2449],[-0.1414, -0.2449],[-0.2828, 0],[-0.1414, 0.2449]]
		self.joint_ranges = [[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]]]
		self.joint_axes = [[[0, 0, 1],[-1, 0.5773, 0],[-1, 0.5773, 0]],[[0, 0, 1],[0, 1, 0],[0, 1, 0]],[[0, 0, 1],[1, 0.5773, 0],[1, 0.5773, 0]],[[0, 0, -1],[1, -0.5773, 0],[1, -0.5773, 0]],[[0, 0, -1],[0, -1, 0],[0, -1, 0]],[[0, 0, -1],[-1, -0.5733, 0],[-1, -0.5773, 0]]]
...
class Hexapod_v16(mujoco_env.MujocoEnv, utils.EzPickle):
	def __init__(self, add_dummy=False, max_segments=3, max_legs=6):
		...
		mujoco_env.MujocoEnv.__init__(self, 'Hexapod_16.xml', 5)
	...
	def step(self, a, goal=[0,1]):
		...
		ctrl_cost = (.5 * np.square(a).sum())/(modelData.num_legs + 3.0)
		survive_reward = 1.0
		if done:
			survive_reward = 0
		...
		for leg_number in range(modelData.num_legs):
			...
			for joint in range(len(modelData.joint_names)):
				...
				if leg_number == 3 and not added_extra:
					...
					leg_joint_pos.append(float(joint_qpos))
					added_extra = True
				if leg_number == 4 and not added_extra:
					joint_qpos_addr = self.model.get_joint_qpos_addr('extra_joint_5')
					joint_qpos = self.data.qpos[joint_qpos_addr]
					leg_joint_pos.append(float(joint_qpos))
					added_extra = True
				if leg_number == 5 and not added_extra:
					joint_qpos_addr = self.model.get_joint_qpos_addr('extra_joint_6')
					joint_qpos = self.data.qpos[joint_qpos_addr]
					leg_joint_pos.append(float(joint_qpos))
					added_extra = True
