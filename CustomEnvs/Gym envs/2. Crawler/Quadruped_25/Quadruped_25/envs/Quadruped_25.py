...
class model_data:
	...
	def __init__(self):
		self.num_legs = 4
		self.leg_lengths = [[0.3, 0.6],[0.3, 0.3, 0.6],[0.3, 0.6],[0.3, 0.3, 0.6]]
		self.leg_starts = [[0.25, 0.55],[0.25, -0.55],[-0.25, -0.55],[-0.25, 0.55]]
		self.joint_ranges = [[[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]]]
...
class Quadruped_v25(mujoco_env.MujocoEnv, utils.EzPickle):
	def __init__(self, add_dummy=False, max_segments=3, max_legs=6):
		...
		mujoco_env.MujocoEnv.__init__(self, 'Quadruped_25.xml', 5)
	...
	def step(self, a, goal=[0, 1]):
		...
		ctrl_cost = (.5 * np.square(a).sum())/(modelData.num_legs+1)
		survive_reward = 1.0
		if done:
			survive_reward = 0
