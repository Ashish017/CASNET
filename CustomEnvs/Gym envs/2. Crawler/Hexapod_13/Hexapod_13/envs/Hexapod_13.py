...
class model_data:
	...
	def __init__(self):
		self.num_legs = 6
		self.leg_lengths = [[0.2828, 0.5656],[0.2828, 0.2828, 0.5656],[0.2828, 0.5656],[0.2828, 0.2828, 0.5656],[0.2828, 0.5656],[0.2828, 0.2828, 0.5656]]
		self.leg_starts = [[0.1414, 0.2449],[0.2828, 0.0],[0.1414, -0.2449],[-0.1414, -0.2449],[-0.2828, 0],[-0.1414, 0.2449]]
		self.joint_ranges = [[[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[0.1745, 1.57]],[[-0.5234, 0.5234],[-0.5234, 0.5234],[0.1745, 1.57]]]
		self.joint_axes = [[[0, 0, 1],[-1, 0.5773, 0]],[[0, 0, 1],[0, 1, 0],[0, 1, 0]],[[0, 0, 1],[1, 0.5773, 0]],[[0, 0, -1],[1, -0.5773, 0],[1, -0.5773, 0]],[[0, 0, -1],[0, -1, 0]],[[0, 0, -1],[-1, -0.5733, 0],[-1, -0.5773, 0]]]
...
class Hexapod_v13(mujoco_env.MujocoEnv, utils.EzPickle):
	def __init__(self, add_dummy=False, max_segments=3, max_legs=6):
		...
		mujoco_env.MujocoEnv.__init__(self, 'Hexapod_13.xml', 5)
	...
	def step(self, a, goal=[0,1]):
		...
		ctrl_cost = (.5 * np.square(a).sum())/(modelData.num_legs + 1.5)
		survive_reward = 1.0
		if done:
			survive_reward = 0
	...
	def _get_obs(self):
		...
		for leg_num in range(modelData.num_legs):
			...
			if leg_num == 1 or leg_num == 3 or leg_num == 5:
