...
class replay_buffer:
	def __init__(self, ob_shape, action_shape):
		...
		self.buffers = {'obs': np.empty([self.size, self.T, ob_shape]),
						'ag': np.empty([self.size, self.T, 3]),
						'g': np.empty([self.size, self.T, 3]),
						'obs_next': np.empty([self.size, self.T, ob_shape]),
						'ag_next': np.empty([self.size, self.T, 3]),
						'actions': np.empty([self.size, self.T, action_shape]),
						}
		self.o_norm = normalizer(ob_shape)
		self.g_norm = normalizer(3)

		self.her_module = her_sampler()

	def store_episode(self, episode_batch):
		mb_obs, mb_ag, mb_g, mb_obs_next, mb_ag_next, mb_actions = episode_batch
